...
def panggil_antrian_teller(slot, engine):
    with open("antrianteller.txt", "r") as file:
        baca = [line.strip() for line in file.readlines()]
    if baca:
        nk = baca.pop(0)
        messagebox.showinfo("Panggilan Antrian", f"Antrian ke-{nk} menuju teller {slot}")
        engine.say(f"Antrian ke-{nk}, menuju teller {slot}")
        engine.runAndWait()
        with open("antrianteller.txt", "w") as file:
            for antrian in baca:
                file.write(antrian + "\n")
    else:
...
def create_gui(queue, slot):
    ...
    root = tk.Tk()
    root.title(f"Teller {slot}")

    label = tk.Label(root, width=30, text=f"Panggil Antrian Selanjutnya")
    label.pack(pady=20)
...
if __name__ == "__main__":
    ...
    while True:
        if not queue1.empty():
            panggil_antrian_teller("A", engine)
            queue1.get()

        if not queue2.empty():
            panggil_antrian_teller("B", engine)
            queue2.get()
